...
const GrupoMateriaCard = ({
  ...
  pendingTask,
  materiaId,
  hasConflict, // ✅ Esta prop viene desde el componente padre
  conflictInfo, // ✅ { materiaConflicto, horarioConflicto }
}) => {
  const navigate = useNavigate();

  const [taskInfo, setTaskInfo] = useState(
    pendingTask
      ? { queueName: pendingTask.queueName, taskId: pendingTask.taskId }
      : { queueName: null, taskId: null }
  );
  ...
  const handleTaskSettled = useCallback(() => {
    const pending = JSON.parse(
      localStorage.getItem("pendingInscriptions") || "{}"
    );
  ...
  if (isEnrolled) {
    ...
    // ✅ Nuevo estado para conflicto de horario
  ...
  const handleInscribir = async () => {
    // ✅ Evitar inscripción si hay conflicto
    if (hasConflict) {
      toast.error(
        `No puedes inscribirte: choque de horario con "${conflictInfo?.materiaConflicto}"`,
        { autoClose: 5000 }
      );
      return;
    }
  ...
  const renderStatus = () => {
    switch (displayStatus) {
      ...
      case "conflict":
        // ✅ Nuevo estado visual para conflicto
        return (
          <div className="status-conflict-wrapper">
            <div className="status-badge status-conflict">
              ...
              <span>Choque de Horario</span>
            </div>
            {conflictInfo && (
              <p className="conflict-message">
                Conflicto con: <strong>{conflictInfo.materiaConflicto}</strong>
                <br />
                <small>{conflictInfo.horarioConflicto}</small>
              </p>
            )}
          </div>
        );
        ...
        <div className="card-header">
          ...
          <div className="header-text">
            ...
            <p className="card-subtitle">
              {grupo.materia.sigla} • Grupo {grupo.grupo}
            </p>
          </div>
        </div>
        ...
        <div className="card-info">
          ...
          <div className="info-row">
            ...
            <span>
              Cupos:{" "}
              <strong
                className={grupo.cupo > 0 ? "text-success" : "text-danger"}
              >
                {grupo.cupo}
              </strong>
            </span>
